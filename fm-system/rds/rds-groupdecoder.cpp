...
#include	"ebu-codetables.c"
	...
	rdsGroupDecoder::rdsGroupDecoder (duoReceiver *RI) {
	MyRadioInterface	= RI;
...
//	Decide by group type code
	switch (grp -> getGroupType()) {
	   ...
	   case RDSGroup::SLOW_LABELING_CODES:
	      break;
...
//	               m_pSink -> addTMCLocationAndEvent (location, event);
	      ...
	      case RDSGroup::EMERGENCY_WARNING:
	         break;

	      case RDSGroup::PROGRAM_TYPE_NAME:
	         break;
	      ...
	      case RDSGroup::ENHANCED_PAGING:
	         break;

	      case RDSGroup::ENHANCED_OTHER_NETWORKS_INFO:
	         break;
	}
...
//	in which case we show the text
	   for (i = 1; i < NUM_OF_FRAGMENTS; i++) {
	      if ((1 << i) == (int32_t) textSegmentRegister + 1)  {
	         setRadioText (prepareText ((char *)textBuffer,
...
//	current segment is received (set bit in segment register to 1)
	...
	setRadioText (prepareText (textBuffer, NUM_OF_CHARS_RADIOTEXT));
...
const QString
	rdsGroupDecoder::prepareText (char *v, int16_t length) {
int16_t	i;

...
/*
 *	Just an old square root formula, now in the std::complex<float> domain
 */
void	cQuadratic (std::complex<float> A, std::complex<float> B, std::complex<float> C,
	                       std::complex<float> *D, std::complex<float> *E) {
std::complex<float>	temp	= sqrt (B * B - cmul (A * C, 4.0));
 ...
 */
	   if (temp [i]. A0 == 0.0) {
	    ...
	    // make coefficients into std::complex<float> numbers
	      A = std::complex<float> (temp [i]. A0, 0.0);
	      B = std::complex<float> (temp [i]. A1, 0.0);
	      C = std::complex<float> (temp [i]. A2, 0.0);
	      cQuadratic (A, B, C, &D, &E);
	    // make required substitutions, then factor again
	      A = std::complex<float> (1.0, 0.0);
	      B = cmul (-D, BW);
	      C = std::complex<float> (Wo * Wo, 0.0);
	      cQuadratic (A, B, C, &D, &E);
	 ...
	 */
	   A = std::complex<float> (temp [i]. B0, 0.0);
	   B = std::complex<float> (temp [i]. B1, 0.0);
	   C = std::complex<float> (temp [i]. B2, 0.0);
	   cQuadratic (A, B, C, &D, &E);
	 // make required substitutions, then factor again
	   A = std::complex<float> (1.0, 0.0);
	   B = cmul (-D, BW);
	   C = std::complex<float> (Wo * Wo, 0);
	   cQuadratic (A, B, C, &D, &E);
	 // final values for coefficients
	   Transfer [2 * i]. B0 = 1.0;
...
//	x1 and x2 generally will be std::complex<float>
//	Substitution is in these first order terms. Substitution will yield
//	quadratic forms with std::complex<float> coefficients.
//	We again can be decomposed into std::complex<float> conjugate pairs that
